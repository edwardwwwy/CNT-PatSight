...
此目录只保存少量可公开复现 CNT-PatSight 八表契约的样例。当前尚未把本地 `data/interim/` 包直接复制到这里；样例必须先完成授权检查、人工复核和脱敏。

## 每个样例应包含

```text
data/samples/<source_id>/
├── source_rights.json
├── source_master.csv
├── source_run.csv
├── catalyst_system.csv
...
- 数据已经人工检查 run 拆分、单位、产率口径、CNT 类型和表间关系；
...
- 运行 `python scripts/validation/validate_tables.py data/samples/<source_id>` 后无校验错误。
