...
class ProductionStore:
    ...
    def _create_schema(self) -> None:
        self.connection.executescript(
            ...
            CREATE TABLE IF NOT EXISTS qwen_tasks (
            ...
            CREATE INDEX IF NOT EXISTS idx_qwen_claim
                ON qwen_tasks(task_status,priority,created_at);
            CREATE TABLE IF NOT EXISTS task_events (
                event_id INTEGER PRIMARY KEY AUTOINCREMENT,
                task_id TEXT NOT NULL REFERENCES qwen_tasks(task_id),
    ...
    def enqueue(
        ...
        cursor = self.connection.execute(
            """
            INSERT OR IGNORE INTO qwen_tasks(
    ...
    def claim(self, worker_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            updated = self.connection.execute(
                """
                UPDATE qwen_tasks SET task_status='running',attempt_count=attempt_count+1,
            ...
            self.connection.commit()
            return dict(self.connection.execute("SELECT * FROM qwen_tasks WHERE task_id=?", (row["task_id"],)).fetchone())
        except Exception:
            self.connection.rollback()
            raise
    ...
    def heartbeat_task(self, task_id: str, worker_id: str) -> None:
        now = utc_now()
        self.connection.execute(
            "UPDATE qwen_tasks SET heartbeat_at=?,updated_at=? WHERE task_id=? AND lease_owner=? AND task_status='running'",
    ...
    def finish_task(
        ...
        *,
        ...
        self.connection.execute(
            """
            UPDATE qwen_tasks SET task_status=?,extraction_disposition=?,request_hash=?,
                raw_result_path=?,last_error=?,lease_owner='',heartbeat_at=?,updated_at=?
            WHERE task_id=?
            """,
            (status, disposition, request_hash, raw_result_path, error, now, now, task_id),
        )
    ...
    def recover_stale(self, stale_before: str) -> int:
        rows = self.connection.execute(
            "SELECT task_id FROM qwen_tasks WHERE task_status='running' AND heartbeat_at < ?",
            (stale_before,),
        ).fetchall()
        now = utc_now()
        for row in rows:
            self.connection.execute(
                """UPDATE qwen_tasks SET task_status='interrupted',lease_owner='',
    ...
    def task_summary(self) -> dict[str, int]:
        return {
            ...
            for row in self.connection.execute(
                "SELECT task_status,COUNT(*) n FROM qwen_tasks GROUP BY task_status"
            )
        }

    def disposition_summary(self) -> dict[str, int]:
        return {
            str(row["extraction_disposition"]): int(row["n"])
            for row in self.connection.execute(
                "SELECT extraction_disposition,COUNT(*) n FROM qwen_tasks GROUP BY extraction_disposition"
            )
        }
